...
LABELS = {
    "en": {
        "title": "OncoAI – TCGA-LUAD Survival Prediction Platform",
        "subtitle": "An interactive AI-powered survival prediction system for lung adenocarcinoma",
    ...
    "cn": {
        "title": "OncoAI – TCGA-LUAD 生存预测",
        "subtitle": "面向肺腺癌个体化生存预测的交互式 AI 临床决策支持工具",
...
def main():
    st.set_page_config(page_title="OncoAI – TCGA-LUAD Survival Prediction", layout="wide")
    ...
    cph, train, train_lp, lp_median = fit_model()
    st.sidebar.write(f"Training rows: {len(train)}")
    st.sidebar.write(f"Median LP cutoff: {lp_median:.3f}")
